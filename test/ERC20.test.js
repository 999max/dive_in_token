...
contract('MaxSecondCoin from ERC20', function (accounts) {
  const [ initialHolder, recipient, anotherAccount ] = accounts;

  const name = 'MaxSecondCoin';
  const symbol = 'MSC';

  const initialSupply = new BN(3000);

  beforeEach(async function () {
    this.token = await MaxSecondCoin.new({ from: initialHolder });
